  ...
  return (
    <div className="categorie container-fluid">
      <header>
        ...
        <div className={`menuButton d-flex align-items-center ${context.bodyWidth<=800?'justify-content-between':'justify-content-end'}`}>
         <SideMenuRes/>              
        <span style={{cursor:'pointer',display:context.bodyWidth<800?'block':'none'}}  data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrolling" aria-controls="offcanvasScrolling" className="fs-3">
          <ion-icon name="menu-outline"></ion-icon>
            ...
            <span className="numOrders1">{context.panier.length}</span>
            ...
            <span className="numFavorite1">{context.favorite.length}</span>
            </div>
          </div>
      ...
      <div className="container row d-flex align-items-center justify-content-between">
        <div className="col-lg-9">
          <div style={{gap:context.bodyWidth<=800 ? '20px' : ''}} className={`filters ${context.bodyWidth<=800 ? 'd-flex justify-content-center' : ''}`}>
            {feature.map((f)=>(
              <div key={f.id} > 
        ...
        <div className="col-lg-3 search " style={{display:context.bodyWidth<=800&&'flex',justifyContent:context.bodyWidth<=800&&'center'}}>
        ...
        {filteredData.length!==0?filteredData.map((game)=>(
            ...
            <div className="item shadow-lg p-1  me-2 mb-3 mt-3 rounded-4 flex-column d-flex" key={game.id}>
                ...
                <div className="featured">
                    ...
                    <button title='add product to your favorite' className={`heart fs-3 ${game.heart&&'text-danger'} ${context.activeDarkMode&&'text-light'}`} style={{background:'none',border:'none',color:context.activeDarkMode&&'white'}}
                      onClick={()=>makeHeart(game)} >
